...
Reads the two source workbooks in data/raw/ and writes:
  ...
  * data/csv/*.csv         flat exports of every table
  * data/csv/per_bank/*    one wide-format CSV per bank
...
PER_BANK_DIR = os.path.join(CSV_DIR, "per_bank")
...
SOURCES = [
    {
        ...
        "bank_type": "PCB",
        "ownership": "Private commercial bank (DSE-listed)",
    },
    {
...
CREATE TABLE banks (
    ...
    bank_type     TEXT NOT NULL,   -- 'PCB' | 'SOCB'
    ownership     TEXT,
...
CREATE TABLE financials (
    ...
    pct_scale        TEXT,            -- 'percent_points' | 'fraction' | ...
...
CREATE TABLE metric_catalog (
    metric_std   TEXT PRIMARY KEY,
    canonical_unit TEXT,
    n_banks      INTEGER,
    n_values     INTEGER,
    raw_labels   TEXT          -- every source label that maps here
);
...
CREATE INDEX idx_fin_bank_year ON financials(bank_id, year);
CREATE INDEX idx_fin_std       ON financials(metric_std, year);
CREATE INDEX idx_fin_instance  ON financials(table_instance);
CREATE INDEX idx_log_bank      ON data_quality_log(bank_id, severity);

-- Authoritative rows only: instance 1, blanks dropped. Start here.
...
FROM financials f JOIN banks b ON b.bank_id = f.bank_id
WHERE f.table_instance = 1 AND f.value_numeric IS NOT NULL;

-- One row per bank-year with the core metrics as columns. Percentages are in
-- percentage points and therefore comparable across every bank.
CREATE VIEW v_bank_year AS
...
def write_db(banks, financials, summaries, notes):
    ...
    conn.executemany(
        "INSERT INTO financials VALUES (:bank_id,:table_instance,:category,:metric,"
        ":metric_std,:unit,:year,:raw_value,:value_numeric,:value_pct_points,"
        ":pct_scale,:value_source,:is_estimated,:source_file,:source_row,:source_col)",
        financials)
    ...
    conn.executemany(
        ...
        summaries)
    conn.executemany(
        "INSERT INTO source_notes VALUES (:bank_id,:note_index,:note_text,:source_row)",
        notes)
    conn.executemany(
    ...
    for r in financials:
        entry = catalog[r["metric_std"]]
        entry["banks"].add(r["bank_id"])
        entry["labels"].add(r["metric"])
        entry["unit"] = entry["unit"] or r["unit"]
        if r["value_numeric"] is not None:
            entry["n"] += 1
    conn.executemany("INSERT INTO metric_catalog VALUES (?,?,?,?,?)", [
        (std, e["unit"], len(e["banks"]), e["n"], " | ".join(sorted(e["labels"])))
...
def export_all(conn):
    for d in (CSV_DIR, PER_BANK_DIR, JSON_DIR):
        os.makedirs(d, exist_ok=True)

    dump_csv(conn, "SELECT * FROM financials ORDER BY bank_id, table_instance, source_row, year",
             os.path.join(CSV_DIR, "all_banks_long.csv"))
    dump_csv(conn, "SELECT * FROM v_bank_year ORDER BY bank_id, year",
             os.path.join(CSV_DIR, "all_banks_bank_year.csv"))
    dump_csv(conn, "SELECT * FROM banks ORDER BY bank_type, bank_id",
             os.path.join(CSV_DIR, "banks.csv"))
    dump_csv(conn, "SELECT * FROM bank_summary_stats ORDER BY bank_id, source_row",
             os.path.join(CSV_DIR, "bank_summary_stats.csv"))
    dump_csv(conn, "SELECT * FROM source_notes ORDER BY bank_id, note_index",
             os.path.join(CSV_DIR, "source_notes.csv"))
    dump_csv(conn, "SELECT * FROM metric_catalog ORDER BY metric_std",
             os.path.join(CSV_DIR, "metric_catalog.csv"))
    dump_csv(conn, "SELECT * FROM data_quality_log ORDER BY severity, bank_id, year",
             os.path.join(CSV_DIR, "data_quality_log.csv"))

    for (bank_id,) in conn.execute("SELECT bank_id FROM banks ORDER BY bank_id"):
        years = [y for (y,) in conn.execute(
            "SELECT DISTINCT year FROM financials WHERE bank_id=? AND table_instance=1 "
            "ORDER BY year", (bank_id,))]
        rows = conn.execute(
            "SELECT metric, unit, year, value_numeric, raw_value FROM financials "
            "WHERE bank_id=? AND table_instance=1 ORDER BY source_row, year", (bank_id,))
        wide, order = {}, []
        for metric, unit, year, val, raw in rows:
            if metric not in wide:
                wide[metric] = {"unit": unit}
                order.append(metric)
            wide[metric][year] = val if val is not None else raw
        with open(os.path.join(PER_BANK_DIR, f"{bank_id}.csv"), "w", newline="",
                  encoding="utf-8") as fh:
            writer = csv.writer(fh)
            writer.writerow(["metric", "unit"] + years)
            for metric in order:
                writer.writerow([metric, wide[metric]["unit"]] +
    ...
    as_json("SELECT * FROM v_bank_year", "bank_year.json")
...
def main():
    banks, financials, summaries, notes = {}, [], [], []
    ...
    conn = write_db(banks, financials, summaries, notes)
    export_all(conn)
